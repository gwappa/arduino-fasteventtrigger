...
#define OUTPUT_MASK (uint8_t)((0x01<<1) | (0x01<<2) | (0x01<<3))

/* intentionally left for reference purposes
// SYNC: PB1 (pin 15)
...
int main(void)
{
	...
	IO_REGISTER |= OUTPUT_MASK;
    PORT_OUT = ~OUTPUT_MASK;
	...
	for (;;)
	{
		...
		if (!(ReceivedByte < 0)){
            ...
            // update
            PORT_OUT = (command << 1) & OUTPUT_MASK;
            CDC_Device_SendByte(&VirtualSerial_CDC_Interface, command);
		}
